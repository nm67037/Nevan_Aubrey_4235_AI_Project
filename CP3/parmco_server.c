 ...
 *
 * FIX:
 * - Added printf statements to log all incoming and
 * outgoing Bluetooth data for debugging.
 * - Changed RFCOMM channel to 22.
 * - Fixed "zombie" service bug with non-blocking socket.
 * - Corrected uint32_t typo.
 * - EDITED: 'x' command now calls stop_all_activity() for a safe stop.
 * - BUG FIX: Correctly handle non-blocking write() errors (EAGAIN)
 * to prevent instant disconnect.
 */
...
// --- THIS IS THE EDITED BLOCK ---
case 'x':
stop_all_activity(); // Call the full, safe stop function
break;
// --- END OF EDITED BLOCK ---
...
// --- THIS IS THE BUG FIX BLOCK ---
int write_ret = write(client_sock, rpm_str, strlen(rpm_str));
if (write_ret < 0) {
    if (errno == EAGAIN || errno == EWOULDBLOCK) {
        // This is not an error, the buffer is just full
        // We will just try again on the next loop
    } else {
        // This is a real error (e.g., client disconnected)
        fprintf(stdout, "Client %s disconnected (write error).\n", buf);
        close(client_sock); 
        break; // Exit inner loop
    }
}
// --- END OF BUG FIX BLOCK ---
